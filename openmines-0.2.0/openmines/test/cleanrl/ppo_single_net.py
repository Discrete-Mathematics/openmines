...
class Args:
    ...
    mine_config: str = "D:/Downloads/openmines-0.2.0/openmines-0.2.0/openmines/src/conf/north_pit_mine.json"
...
class CheckpointManager:
    ...
    def save_checkpoint(self,
        ...
        # 清理文件名中的非法字符
        #filename = sanitize_filename(filename)  # 添加这行
        ...
        if is_best:
            ...
            best_path = os.path.join(self.checkpoint_dir, f'best_{safe_filename}')
...
if __name__ == "__main__":
    ...
    #args.env_id = sanitize_filename(args.env_id)


    safe_env_id = args.env_id.replace("/", "_").replace("\\", "_")

    hyperparam_str = (
        f"lr{args.learning_rate:.2e}_e{args.ent_coef:.2e}_g{args.gamma:.3f}_"
        f"c{args.clip_coef:.2f}_l{args.gae_lambda:.3f}_ep{args.update_epochs}_"
        f"gr{args.max_grad_norm:.2f}_hs{args.hidden_size}_ns{args.num_steps}_"
        f"ne{args.num_envs}_mb{args.num_minibatches}_rm{args.r_mode}"
    )
    hyperparam_hash = hashlib.md5(hyperparam_str.encode()).hexdigest()[:8]  # 取前8位

    run_name = (
        f"{safe_env_id}__{args.exp_name}__"
        f"s{args.seed}__{hyperparam_hash}__"
        f"t{int(time.time())}"
    )
    ...
    #norm_path = manage_normalization_params(args=args)
    norm_path = "D:/Downloads/openmines-0.2.0/openmines-0.2.0/datasets/dispatch_data_20260312_164235/normalization_params.json"
    ...
    max_avg_tons = 0
    avg_tons = 0
    latest_avg_raw_reward = 0.0  # 新增：追踪最新episode的reward
    ...
    for iteration in range(start_iteration + 1, args.num_iterations + 1):
        ...
        if iteration % args.save_interval == 0 or is_best:
            checkpoint_manager.save_checkpoint(
                ...
                optimizer,
                iteration,
                latest_avg_raw_reward,   # 修复：使用真实reward而非永远为0的current_raw_reward
                is_best,
                additional_info={
        ...
        for step in range(args.num_steps):
            ...
            for idx, (term, trunc) in enumerate(zip(terminations, truncations)):
                if term or trunc:
                    ...
                    latest_raw_episode_reward = raw_episode_reward
                    latest_avg_raw_reward = raw_episode_reward  # 新增：更新最新reward
        ...
        # 记录日志
        writer.add_scalar(f"charts/episodic_return_{args.r_mode}", latest_raw_episode_reward, global_step)
        if args.r_mode == "reward_norm":
            normalized_reward = agent.normalize_reward(torch.tensor(latest_raw_episode_reward, device=device))
    ...
    checkpoint_manager.save_checkpoint(
        ...
        optimizer,
        args.num_iterations,
        latest_avg_raw_reward,  # 修复：最终保存也使用真实reward
    ...
    print("\n训练完成!")
    print(f"最佳产量: {max_avg_tons:.2f} 吨")
    print(f"最佳原始奖励: {latest_avg_raw_reward:.2f}")
    print(f"最佳正则化奖励: {agent.normalize_reward(torch.tensor(latest_avg_raw_reward, device=device)).item():.2f}")